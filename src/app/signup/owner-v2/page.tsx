...
function OwnerSignupV2Form() {
  ...
  const [companyInviteLabel, setCompanyInviteLabel] = useState('')
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const label = String((data as { name?: string }).name || '').trim()
      setCompanyInviteLabel(label)
    })()
        ...
        <div style={{ marginBottom: 24, textAlign: 'center' }}>
          ...
          {companyId ? (
            <div style={{ marginTop: 10, fontSize: 12, color: PURPLE, lineHeight: 1.55 }}>
              {companyInviteLabel
                ? `${companyInviteLabel} 제휴 초대로 가입 중이에요`
                : '제휴 초대로 가입 중이에요'}
            </div>
          ) : null}
          {ref ? (
