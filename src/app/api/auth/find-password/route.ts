...
export async function POST(req: NextRequest) {
  try {
    const { userId, phone, step, code, newPassword } = await req.json()

    if (step === 'send') {
      const cleanPhone = String(phone || '').replace(/\D/g, '')
      if (!userId?.trim() || !cleanPhone || cleanPhone.length < 10) {
        return NextResponse.json({ error: '아이디와 휴대폰 번호를 입력해주세요' }, { status: 400 })
      }
      ...
      if (!user || user.phone !== cleanPhone) {
        return NextResponse.json({ error: '아이디 또는 휴대폰 번호가 일치하지 않아요' }, { status: 404 })
      }
    ...
    if (step === 'reset') {
      ...
      const { data: record } = await adminSupabase
        ...
        .maybeSingle()
      if (!record) {
        return NextResponse.json({ error: '인증을 먼저 완료해주세요' }, { status: 400 })
      }
      const email = userId.includes('@') ? userId : `${userId.trim()}@auran.kr`
      const { data: user } = await adminSupabase
        .from('users')
      ...
      const { error } = await adminSupabase.auth.admin.updateUserById(user.auth_id, {
        password: newPassword,
      })
      if (error) {
        return NextResponse.json({ error: '비밀번호 변경에 실패했어요' }, { status: 500 })
      }
      await adminSupabase.from('auth_verification_codes').delete().eq('id', record.id)
      return NextResponse.json({ ok: true })
    }
