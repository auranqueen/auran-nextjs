...
export async function POST(req: NextRequest) {
  ...
  if (!invoiceId) {
    return NextResponse.json({ ok: false, error: 'invoice_id_required' }, { status: 400 })
  }
  ...
  const amount = Math.trunc(Number(invoice.total_amount))
  ...
  const postdata: Record<string, string> = {
    ...
    linkkey,
    linkval,
    goodname: `${shopname} ${monthLabel} 월청구서`,
    price: String(amount),
    recvphone,
    memo: `brand_self_invoice ${monthLabel}`,
    smsuse: 'n',
    reqaddr: '0',
    feedbackurl: `${base}/api/payments/brand-self/civasan/webhook`,
    returnurl: `${base}/dashboard/owner/brand-orders/invoice?paid=1`,
    checkretry: 'y',
    skip_cstpage: 'y',
    var1: intent.id,
    var2: invoiceId,
