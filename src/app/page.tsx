...
const HORMONE_PHASE_TIP_ROWS: [string, string][] = [
  ...
  ['만개기', '호르몬이 최고조예요! 피부가 가장 빛나는 황금기예요 🌟'],
  ['황금기', '생리 끝나고 회복되는 시기예요. 피부가 서서히 맑아져요'],
  ['물들기', '생리 전 시기예요. 트러블이 생기기 쉽고 피부가 칙칙해질 수 있어요'],
  ['달빛기', '생리 중인 시기예요. 피부가 예민하고 붓기 쉬워요'],
]
...
export default function CustomerHomePage() {
  ...
  const [checkinResult, setCheckinResult] = useState<{ toast_earned: number; streak: number; message: string } | null>(null)
  ...
  useEffect(() => {
    if (!mounted) return
    const todayKey = 'auran_checkin_' + new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Seoul' })
    ...
    void (async () => {
      try {
        ...
        if (json.ok && !json.already) {
          setCheckinResult({ toast_earned: json.toast_earned || 0, streak: json.streak || 1, message: json.message || '' })
  ...
  useEffect(() => {
    ...
    void supabase
      .from('products')
      .select('id, name, sale_price, retail_price, thumb_img, step_tags, func_tags, brands(name)')
      .eq('brand_id', selectedBrand.id)
      .eq('status', 'active')
      .then(({ data }) => {
        if (cancelled) return
        setBrandProducts((data as any[]) || [])
        setBrandProductsLoading(false)
      })
    return () => {
      cancelled = true
  ...
  const skinRecList = useMemo(() => {
    ...
    // gender 필터
    const userGender = (motivationProfile as any)?.gender ?? ''
    if (userGender === 'male') {
      const g = skinRecPool.filter((p: any) => {
        const gt = String(p.gender_tag ?? '공용')
        return gt === '남성' || gt === '공용' || gt === ''
      })
      if (g.length > 0) skinRecPool = g
    } else if (userGender === 'female') {
      const g = skinRecPool.filter((p: any) => {
        const gt = String(p.gender_tag ?? '공용')
        return gt === '여성' || gt === '공용' || gt === ''
      })
      if (g.length > 0) skinRecPool = g
    }
    // age 점수 가산
    const birthDate = (motivationProfile as any)?.birth_date
    if (birthDate) {
      const age = new Date().getFullYear() - new Date(birthDate).getFullYear()
      const ageTag = age < 20 ? '10대' : age < 30 ? '20대' : age < 40 ? '30대' : age < 50 ? '40대' : '50대이상'
      skinRecPool = skinRecPool.map((p: any) => {
        const tags: string[] = Array.isArray(p.age_tag) ? p.age_tag : []
        const bonus = tags.includes(ageTag) || tags.includes('전연령') ? 2 : 0
        return { ...p, _score: (p._score ?? 0) + bonus }
  ...
  const profileSkinConcerns = Array.isArray((motivationProfile as any)?.skin_concerns)
    ? (motivationProfile as any).skin_concerns.map((k: string) => CONCERN_LABEL_MAP[k] ?? k).join(', ')
  ...
  <div style={{
    ...
    <div style={{
      ...
      {showCheckinPopup && checkinResult && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 999, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', pointerEvents: 'none' }}>
          <div style={{ width: '100%', maxWidth: 480, background: 'var(--bg, #0d0d0d)', borderRadius: '20px 20px 0 0', padding: '20px 20px 36px', pointerEvents: 'auto', boxShadow: '0 -4px 30px rgba(0,0,0,.4)', border: '0.5px solid rgba(255,255,255,.08)' }}>
            <div style={{ width: 36, height: 4, background: 'rgba(255,255,255,.15)', borderRadius: 2, margin: '0 auto 18px' }} />
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 40, marginBottom: 8 }}>🍞</div>
              <div style={{ fontSize: 16, fontWeight: 500, color: '#F0E8FF', marginBottom: 4 }}>출석 완료!</div>
              <div style={{ fontSize: 13, color: '#9B7EC8', marginBottom: 12 }}>{checkinResult.streak}일 연속 출석 중이에요</div>
              <div style={{ display: 'inline-block', background: 'rgba(201,169,110,.15)', border: '0.5px solid rgba(201,169,110,.3)', color: '#C9A96E', fontSize: 15, fontWeight: 500, padding: '6px 18px', borderRadius: 20, marginBottom: 16 }}>
                +{checkinResult.toast_earned}T 적립됐어요 💜
              </div>
              {checkinResult.streak === 6 && <div style={{ fontSize: 11, color: '#C9A96E', marginBottom: 12 }}>내일도 오시면 7일 연속 보너스 +500T!</div>}
              {checkinResult.streak === 29 && <div style={{ fontSize: 11, color: '#C9A96E', marginBottom: 12 }}>내일 30일 개근 달성! +3,000T 보너스!</div>}
              <button onClick={() => setShowCheckinPopup(false)} style={{ width: '100%', padding: '12px', borderRadius: 12, background: '#7B5EA7', border: 'none', color: '#fff', fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>
                확인 💜
              </button>
            </div>
          </div>
