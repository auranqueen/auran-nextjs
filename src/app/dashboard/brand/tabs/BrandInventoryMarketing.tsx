...
export default function BrandInventoryMarketing({ brandId }: Props) {
  ...
  const doSend = async () => {
    ...
    setSaving(true)
    const evtInfo = EVT_TYPES.find(e => e.key === selEvt)
    const { error } = await supabase.from('brand_messages').insert({
      brand_id: brandId,
      message_type: 'auto_order',
      target_type: 'all',
      title: `${evtInfo?.icon} ${evtInfo?.label} — ${popup.lot.product_name}`,
      body: msgText,
      send_count: 1,
    })
    if (!error) {
      await supabase.from('brand_posts').insert({
        brand_id: brandId,
        title: `[이벤트] ${evtInfo?.label} — ${popup.lot.product_name}`,
        body: msgText,
        is_pinned: false,
        author_type: 'brand',
        reply_count: 0,
  ...
  const LotCard = ({ lot, urgent }: { lot: LotRow; urgent: boolean }) => (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '0.5px solid rgba(255,255,255,0.05)' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2, flexWrap: 'wrap' as const }}>
          ...
          <span style={{ fontSize: 11, color: dColor(lot.days) }}>{dIcon(lot.days)} {lot.days !== null ? `D-${lot.days}` : '기한없음'}</span>
        </div>
      ...
      <button type="button" onClick={() => openPopup(lot)}
        style={{ fontSize: 12, padding: '7px 12px', borderRadius: 7, border: 'none', background: urgent ? DANGER : lot.days !== null && lot.days <= 180 ? GOLD : PURPLE, color: '#fff', cursor: 'pointer', flexShrink: 0, whiteSpace: 'nowrap' as const, display: 'flex', alignItems: 'center', gap: 4 }}>
        😊 기분좋게 처리하기
      </button>
    </div>
  )
  return (
    <div>
      ...
      {viewMode === 'normal' && (
        <div style={CARD}>
          <div style={{ fontSize: 12, color: SUB, marginBottom: 10 }}>🟢 정상 재고 시즌 기획 이벤트</div>
          {normalLots.length === 0
            ? <div style={{ textAlign: 'center', padding: '20px 0', color: SUB, fontSize: 13 }}>정상 재고 로트가 없어요</div>
            : normalLots.map(lot => <LotCard key={lot.id} lot={lot} urgent={false} />)}
        </div>
      )}
      {viewMode === 'bundle' && (
        <div style={CARD}>
          ...
          {lots.map(lot => (
            <div key={lot.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: '0.5px solid rgba(255,255,255,0.05)' }}>
              <input type="checkbox" disabled={lot.days !== null && lot.days <= 30}
                style={{ flexShrink: 0, accentColor: PURPLE, width: 16, height: 16 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' as const }}>
                  ...
                  {lot.days !== null && lot.days > 90 && lot.days <= 180 && <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 8, background: 'rgba(201,169,110,0.1)', color: GOLD }}>선택 가능</span>}
          ...
          <button type="button" onClick={() => showToast('번들 패키지 구성 — 추후 구현 예정')}
            style={{ width: '100%', padding: '10px', borderRadius: 8, border: 'none', background: PURPLE, color: '#fff', fontSize: 13, cursor: 'pointer', marginTop: 12 }}>
            🎀 번들 패키지 이벤트 생성
          </button>
        </div>
      )}
      {popup.open && popup.lot && (
        <div onClick={e => { if ((e.target as HTMLElement).id === 'mkt-overlay') closePopup() }}
          id="mkt-overlay"
          ...
          <div style={{ background: '#1a1520', borderRadius: 16, padding: 20, width: '100%', maxWidth: 420, maxHeight: '85vh', overflowY: 'auto', border: '0.5px solid rgba(255,255,255,0.1)' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: SUB }}>{popup.lot.lot_number} · 잔여 {popup.lot.remaining_qty.toLocaleString()}개{popup.lot.days !== null ? ` · D-${popup.lot.days}` : ''}</div>
              </div>
              <button type="button" onClick={closePopup} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: SUB, lineHeight: 1, padding: 4 }}>✕</button>
            </div>
            ...
            {step === 2 && selEvt && (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                  <button type="button" onClick={() => setStep(1)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: SUB, fontSize: 13, padding: 0 }}>← 뒤로</button>
                  <span style={{ fontSize: 13, fontWeight: 500, color: TEXT }}>{EVT_TYPES.find(e => e.key === selEvt)?.icon} {EVT_TYPES.find(e => e.key === selEvt)?.label}</span>
                </div>
            ...
            {step === 3 && (
              <div style={{ textAlign: 'center', padding: '10px 0' }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 500, color: TEXT, marginBottom: 6 }}>기분좋게 완료! 💜</div>
                <div style={{ fontSize: 12, color: SUB, lineHeight: 1.9, marginBottom: 16 }}>
                  {popup.lot?.product_name} · {EVT_TYPES.find(e => e.key === selEvt)?.label}<br/>마감일: {deadline}
                </div>
                <div style={{ background: 'rgba(76,175,80,0.08)', border: '0.5px solid rgba(76,175,80,0.2)', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#4CAF50', lineHeight: 1.9, textAlign: 'left' as const, marginBottom: 16 }}>
                  ✅ 오렌톡 자동 발송됨<br/>✅ 커뮤니티 공지 등록됨<br/>✅ 소진 현황 실시간 연동
                </div>
                <div style={{ fontSize: 12, color: '#c4a7e7', marginBottom: 16 }}>쓰린 마음이 기분좋게 바뀌는 중... 💜</div>
