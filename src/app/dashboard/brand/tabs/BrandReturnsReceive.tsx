...
const CONDITIONS = ['정상품', '파손·불량', '유통기한 문제', '기타'] as const
const PROCESSES = ['재고 반영', '폐기 (본사승인 필요)'] as const
interface ReturnRow {
  id: string
  type: string
  reason_code: string
    ...
    if (!error) {
      if (isRestock && matched.inventory_id) {
        ...
        await supabase.from('brand_stock_logs').insert({
          ...
          memo: `반품 수령: ${matched.reason_code} · ${condition}`,
        })
      }
      await supabase.from('brand_messages').insert({
        ...
        target_type: 'all',
        title: `반품 수령 완료`,
        body: `반품 제품이 수령됐어요. 상태: ${condition} · 처리: ${isRestock ? '재고 반영' : '폐기 대기 (본사 확인 필요)'}`,
        send_count: 1,
      })
      setDone(true)
      showToast(isRestock ? '수령 완료! 재고 자동 반영됨' : '수령 완료! 폐기 승인 대기 중')
      void loadPending()
    } else {
  ...
  return (
    <div>
      ...
      <div style={{ ...CARD, borderColor: 'rgba(201,169,110,0.3)' }}>
        ...
        <div style={{ fontSize: 11, color: SUB }}>코드 없는 반품은 수령 불가 · 코드는 본사 승인 후 발급됩니다</div>
      </div>
      {matched && !done && (
        <>
          ...
          <div style={CARD}>
            ...
            <div style={{ padding: '8px 10px', background: 'rgba(229,57,53,0.06)', border: '0.5px solid rgba(229,57,53,0.2)', borderRadius: 6, fontSize: 11, color: 'rgba(229,57,53,0.7)', marginBottom: 10, lineHeight: 1.6 }}>
              🔒 처리 후 수정 불가 · 담당자 이름 자동 기록 · 본사 자동 통보
            </div>
      ...
      {done && (
        <div style={{ ...CARD, borderColor: 'rgba(76,175,80,0.3)' }}>
          ...
          <button type="button" onClick={() => { setDone(false); setMatched(null); setCodeInput(''); setCondition(''); setProcess(''); setStaffName('') }}
