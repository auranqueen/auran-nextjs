...
export function OwnerBrandSelfTierSection({
  ...
  const purchase = async (brand: SelfTierBrand, tierPackageId: string) => {
    ...
    try {
      ...
      if (json?.ok && json?.demo) {
        setToast(`${json.grade || '등급'} 체험 활성화 완료!`)
        router.refresh()
        return
      }
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        disabled={busy === p.id}
                        onClick={() => void purchase(b, p.id)}
                        style={{
                          fontSize: 11,
                          padding: '4px 10px',
                          borderRadius: 8,
                          border: 'none',
                          background: b.payappActive ? '#7B5EA7' : 'rgba(123,94,167,0.35)',
                          color: '#fff',
                          cursor: 'pointer',
                          flexShrink: 0,
                        }}
