...
export default function ChartsSection() {
  ...
  return (
    <div style={{ background: BG, color: TEXT, maxWidth: 1024, margin: '0 auto', paddingBottom: 40 }}>
      <div style={{ padding: 16 }}>
        ...
        <div style={{ marginTop: 20, background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ display: 'flex', borderBottom: `1px solid ${BORDER}` }}>
            {([
              ['oraen', '오렌 예약 고객', '예약과는 달라요! 오렌 앱에 가입한 고객 중 시술차트 기록이 있는 분들이에요'],
              ['visit', '내방 고객', '직접 등록하거나 오프라인으로 방문한 고객이에요'],
              ...
              >
                <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                  {label}
                  <span title={hint} style={{ fontSize: 12, color: SUB, cursor: 'help', fontWeight: 400 }}>❓</span>
                </span>
              </button>
            ))}
          </div>
