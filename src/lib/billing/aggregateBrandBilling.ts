...
export async function aggregateBrandBilling(
  ...
  const { data: orderRows, error: orderErr } = await supabase
    .from('brand_orders')
    .select('id, total_amount, points_earned, points_used, points_used_reward, status, items')
  ...
  const pouchBasisAmount = Math.max(0, rawTotalAmount - pointsUsedThisCycle - samplePouchAmount)
