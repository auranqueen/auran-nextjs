...
> **브랜드 뱃지/등급 시스템은 시바산 4단계 이름을 하드코딩하지 않는다. 등급명·개수·커미션율은 전부 brand_tier_packages 테이블 값(동적)만 사용.**
...
> **브랜드 자체 결제(`brand-self/*`)는 오렌 결제 시스템과 완전 분리 — 브랜드마다 API/웹훅 파일 전부 새로 만들 것(재사용 최소화), 커미션 로직 절대 포함 금지**

> **브랜드 발주 프로모션은 `supply_promos` DB 조회 방식(GRADE_PROMOS 하드코딩 폐기됨). 단가는 `products.supply_price`, 0이면 발주 불가.**
