...
- feat: 전성분 텍스트 기반 AI 분석 버튼 추가 (사진 없이 텍스트로 분석 가능)

## 2026-09-23
- feat: 제품 상세페이지 JSON-LD Product schema 추가 (구글 리치결과)
...
- feat: 수동 태그 칩 카테고리별 분기 (바디/두피/헤어/이너/기기 전용 칩 목록)
...
- perf: admin/points 브라우저 Supabase 직접호출 2건 제거 → /api/admin/point-settings 서버API 전환
...
- perf: admin/templates 브라우저 Supabase 직접호출 4건 제거 → /api/admin/bundle-templates 서버API 전환
...
- perf: admin/skinstar 브라우저 Supabase 직접호출 4건 제거 → /api/admin/skinstar-videos 서버API 전환
...
## 2026-09-08
### ui: 살롱 상세 탭을 아이콘 카드형으로 변경

- `salons/[id]/page.tsx`: 텍스트+밑줄 탭을 lucide 아이콘 카드(활성 보라 채움)로 교체. `selectTab`·`?tab=` URL 동기화 유지
- 아이콘: 시술 메뉴 Scissors · 브랜드 제품 Package · 스토리 Camera · 리뷰 Star · 샵 정보 Info
...
- `/api/owner/education/sessions`에 `applied_count` 포함(진행바 신청인원/정원).
- 쉽게: 세션을 누르면 자세히 보고, 거기서 신청할 수 있다.
...
- 쉽게: 「원장님 상담」으로 통일하고, 들어가면 채팅 목록이 먼저 보인다.
### feat: 원장 홈(v3) 브랜드 상담 아코디언 위젯
- OwnerHomeV3: 고객 상담톡 아코디언 바로 아래 「🏢 브랜드 상담」 추가. `/api/owner/chat/channels`로 최근 3건(회사명·로고·미리보기), `unread_by_owner` 합계 뱃지, 「전체 브랜드상담 보기 →」→ `/dashboard/owner/brand-chat`.
- 쉽게: 원장 홈에서도 브랜드사와 1:1 대화를 바로 확인할 수 있다.
### chore: 레거시 원장 홈 v2 완전 제거
- `page.tsx` `?v=2` 분기 삭제. 진입 링크 5곳(services/reviews/programs/BookingManagePage/웰컴 알림)을 `/dashboard/owner`(v3)로 통일.
- 삭제: `client-v2.tsx`, `OwnerV2LowerStack.tsx`, `client-v2-helpers.ts`. v1(`?v=1`) 레거시는 유지.
- 빌드 기준 `/dashboard/owner` 번들 21.7kB → 11.6kB.
- 쉽게: 안 쓰던 옛날 원장 홈 화면을 걷어내고, 모두 v3 기본 홈으로 모았다.
...
- 기존 honey_toast·customer_toast_amount·toast_transactions/increment_* 재사용. 신규 정산시스템 없음.
- 쉽게: 오렌씬으로 결제하면 팝업·원장알림·업로더 예상/확정 적립알림이 예약·제품 모두 이어진다.
...
### ui: 스토어꾸미기 배너 제목 변경 + 슬롯 삭제
- store-decoration/page.tsx: 제목 「프로모션 배너 관리」. 삭제 확인 후 해당 슬롯 초기화하고 기존 배너 저장과 같은 update. 안내문구·업로드 로직 미변경.
- 쉽게: 배너 이름을 프로모션 배너 관리로 바꾸고, 선택한 칸을 지워서 바로 저장할 수 있다.

### ui: 스토어꾸미기 배너 링크 칩을 없음/URL만 유지
- store-decoration/page.tsx: BANNER_LINK_OPTIONS에서 booking/chat 제거. 저장된 booking/chat은 state에 유지하고 칩 표시만 none. 살롱홈 클릭 분기 미변경.
- 쉽게: 배너 링크는 없음 또는 외부 URL만 고를 수 있고, 예전에 예약/상담으로 둔 값은 저장을 다시 누르기 전까지 그대로다.
...
- 쉽게: 스토리는 살롱홈 탭에서 보고, 브랜드제품 페이지는 상품만 본다.
...
### style: 적립포인트·아레테포인트 카드를 골드/퍼플 투카드로 배치
- AreteMembershipCard: 가로 flex gap 10. 적립은 골드 그라데이션(#faf3e6/#ecdfc4), 아레테는 퍼플(#f1ecf7/#e2d5f0). 숫자 표시를 위해 brand_points REWARD 잔액만 기존 ARETE 조회와 병렬로 읽음. 결제·인보이스·꾸러미 로직 미변경.
- 쉽게: 적립포인트와 아레테포인트 카드를 골드톤/퍼플톤으로 나란히 예쁘게 꾸몄다.
