...
- **원장 적립금(T) CSV 대량업로드**: `parseCsv.ts` 공통 파서, `POST /api/brand/owner-points/bulk-import`(brand/admin 인증, service role로 ledger+balance 갱신, `manual_init` idempotency). 브랜드 대시보드 `BrandTabOwners` — CSV 업로드·dry-run·T 잔액 표시. **093 마이그레이션·company_id 백필 선행 필요**
